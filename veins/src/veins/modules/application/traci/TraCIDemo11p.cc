...
void TraCIDemo11p::handleSelfMsg(cMessage* msg)
{
    if (TraCIDemo11pMessage* wsm = dynamic_cast<TraCIDemo11pMessage*>(msg)) {
        ...
        if (wsm->getSerial() != 4){
            // pablo send accident msg
            DemoBaseApplLayer::controlMessage(wsm, "tx", simTime().dbl());
        }
...
void TraCIDemo11p::handlePositionUpdate(cObject* obj)
{
    ...
    if (mobility->getSpeed() < 1 && findHost()->getIndex() == 1) {  // pablo modify 1 node[1] in omnetpp.ini
